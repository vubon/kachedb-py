...
class AsyncKacheClient:
    ...
    async def auth(self, password: str, username: str | None = None) -> bool:
        """Authenticate with the KacheDB server asynchronously.
    ...
    async def bgrewriteaof(self) -> str:
        """Rewrite the append-only file in the background asynchronously."""
        result = await self._execute("BGREWRITEAOF")
        if isinstance(result, bytes):
            return result.decode("utf-8", errors="replace")
        return str(result)

    async def vindex_create(
        self,
        name: str | bytes,
        dim: int,
        m: int = 16,
        ...
        """Create a new HNSW vector index asynchronously.
    ...
    async def vindex_drop(self, name: str | bytes) -> bool:
        """Drop a vector index asynchronously.

        Returns ``True`` if the index was found and deleted, ``False`` otherwise.
        """
        result = await self._execute("VINDEX", "DROP", name)
        return bool(result == 1)

    async def vindex_info(self, name: str | bytes) -> dict[str, Any] | None:
        """Get detailed metadata and statistics for a vector index asynchronously."""
        raw = await self._execute("VINDEX", "INFO", name)
        if not isinstance(raw, list):
            return None
        info: dict[str, Any] = {}
        for i in range(0, len(raw) - 1, 2):
